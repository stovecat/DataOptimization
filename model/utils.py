...
def get_device(model_name=None):
    if torch.cuda.is_available():
        device_num = np.random.randint(torch.cuda.device_count())
...
def get_ckptdir(net_name, net_structure, sigma=None, log=False):
...
def load_train_vali_data(data_fold, small_dataset=False, data_type='mslr-web30k'):
    """
...
def eval_cross_entropy_loss(model, device, loader, epoch, writer=None, phase="Eval", sigma=1.0):
    ...
    with torch.set_grad_enabled(False):
        ...
        for X, Y in loader.generate_batch_per_query(loader.df):
            ...
            X_tensor = X.to(torch.float32)#torch.Tensor(X).to(device)
            y_pred = model(X_tensor)
            if True:
                y_pred_diff = torch.sigmoid(y_pred - y_pred.t())
            else:
...
#             rels.append(rel)
        ...
        for X, Y in valid_loader.generate_batch_per_query():
            ...
            result_qid = sorted([(s, r) for s, r in zip(score, rels)], key=lambda x: x[0], reverse=True)
