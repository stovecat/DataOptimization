...
class RankNet(nn.Module):
    def __init__(self, net_structures, double_precision=False):
        ...
        def none_activation(input):
            return input
        self.activation = none_activation#nn.ReLU6()
        self.elu = nn.ELU()

    def forward(self, input1):
        for i in range(1, self.fc_layers):
            fc = getattr(self, 'fc' + str(i))
            input1 = self.elu(fc(input1))
...
def train_rank_net(
    ...
    net, net_inference, ckptfile = get_train_inference_net(
        train_algo, train_loader.num_features, start_epoch, double_precision, log
    )
    device = get_device('RankNet')
    net.to(device)
...
def get_train_inference_net(train_algo, num_features, start_epoch, double_precision, log=False):
    ...
    if train_algo == BASELINE:
        ...
        ckptfile = get_ckptdir('ranknet', ranknet_structure, log=log)
...
def baseline_pairwise_training_loop(
    ...
    for x_i, y_i, x_j, y_j in train_loader.generate_query_pair_batch(batch_size, shuffle=True):
...
#     if False and train_loader.influences_on:
#         threshold = 0#min([min(_infl) for _infl in train_loader.influences])
#         query_level = True
#         if query_level:
#             influences_val = [v for v in train_loader.influences]
#         else:
#             influences_val = []
#             for v in train_loader.influences: influences_val.extend(v)
            
#         influences_val = np.array(influences_val)
#         sorted_influences = sorted(influences_val)
# #         print(sorted_influences[:10])
#         threshold = sorted_influences[9]
#         with open('model/ckptdir/'+'30_100_CE2_influences'+'/query_influences_clean.pkl', 'rb') as fp:
#             infl_clean = pickle.load(fp)
#     if True:
#         query_level = False
#         #with open('clean_influence.pkl', 'rb') as fp:
#         with open('partial_clean_influence.pkl', 'rb') as fp:
#             infl_clean = pickle.load(fp)
    
    for X, Y in train_loader.generate_batch_per_query(shuffle=True):
        #if X is None or X.shape[0] == 0 or (query_level and train_loader.influences[train_loader.current_idx] <= threshold):
        #if X is None or X.shape[0] == 0 or (query_level and train_loader.influences[train_loader.current_idx] < infl_clean[train_loader.current_idx]):
        ...
        if False:
            ...
            if train_loader.current_idx in train_loader.drop_documents.keys():
                for drop_doc_idx in train_loader.drop_documents[train_loader.current_idx]:
                    ...
                    rel_diff[:, drop_doc_idx] = 0
#             if False and train_loader.influences_on and not query_level:
#                 infl = np.array(train_loader.influences[train_loader.current_idx]).reshape(-1, 1)
#                 infl = (infl > threshold).astype(np.int)
#                 infl_diff = infl * infl.T
...
#             if False: #infleunce filter
#                 infl = torch.tensor(infl_clean[train_loader.current_idx])
#                 assert len(infl) == num_pairs
#                 threshold = torch.sort(infl, dim=0, descending=False)[0][min(num_pairs.int()-1, train_loader.top_n)]
#                 infl_diff = (infl > threshold).to(torch.float32)
#             if True: #random filter
#                 tmp_top_n = min(num_pos_pairs.int().item()-1, train_loader.top_n)
#                 rand_filter = [0.]*tmp_top_n + [1.] * (num_pos_pairs.int().item()-tmp_top_n)
#                 random.shuffle(rand_filter)
#                 infl_diff = torch.tensor(rand_filter+rand_filter)
        ...
        if training_algo == SUM_SESSION:
            ...
            if False:
                C = torch.cat([torch.masked_select(C_pos, pos_pairs.bool()).view(-1), 
                               torch.masked_select(C_neg, neg_pairs.bool()).view(-1)], dim=0)
                assert C.size() == infl_diff.size()
                C = C * infl_diff
            else:
                C = pos_pairs * C_pos + neg_pairs * C_neg
            if False and train_loader.influences_on and not query_level:
                C = C * torch.tensor(infl_diff)
            loss += torch.sum(C)
...
def load_from_ckpt(ckpt_file, epoch, model, log=True):
